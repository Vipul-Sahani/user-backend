...
import com.example.RentalService.DTO.LoginDTO;
import com.example.RentalService.DTO.UserRegisterDTO;
import com.example.RentalService.model.Users;
import com.example.RentalService.repo.UserRepositry;
...
public class AuthService {
     ...
     * Registers a new user by encoding the password and saving it in the database.
     * 
     * @param userRegisterDTO The user details for registration.
     * @return The saved user object.
     */
     ...
     * @return ResponseEntity containing the JWT token and success message if authentication succeeds, otherwise an error message.
     */
    public ResponseEntity<?> verify(LoginDTO loginDTO, HttpServletResponse response) {
        Authentication authentication = authManager.authenticate(
            new UsernamePasswordAuthenticationToken(loginDTO.getEmail(), loginDTO.getPassword()));

        if (authentication.isAuthenticated()) {

            Users userDemo = repo.findByEmail(loginDTO.getEmail()).get();
            String jwtToken = jwtService.generateToken(
                    String.valueOf(userDemo.getId()),
                    userDemo.getEmail(),
                    userDemo.getRole().toLowerCase());
